...
|---|---|---|---|
| DAG canvas sync | HIGH | Fixed | Medium-High |
| Projection opening | HIGH | Fixed (verified) | Low |

---

## Implementation Log (2026-01-28)

### DAG Sync Fixes Implemented

#### 1. Replaced setTimeout(0) with queueMicrotask
**File**: `frontend/src/components/editors/PlanVisualEditor/hooks/usePlanDagCQRS.ts`

Changed the `isExternalSync` flag reset from macrotask (`setTimeout(0)`) to microtask (`queueMicrotask()`) for tighter timing, reducing the window for missed subscription updates.

#### 2. Added Subscription Refresh After Drag Ends
**File**: `frontend/src/components/editors/PlanVisualEditor/hooks/usePlanDagCQRS.ts`

- Added `pendingRefresh` flag to `editorStateRef.sync`
- When subscription arrives during drag, sets `pendingRefresh = true`
- When `setDragging(false)` is called, checks `pendingRefresh` and triggers `refreshData()` after 200ms delay
- This recovers from subscription updates missed during drag operations

#### 3. Added Manual Refresh Button to Toolbar
**Files**:
- `frontend/src/components/editors/PlanVisualEditor/components/AdvancedToolbar.tsx`
- `frontend/src/components/editors/PlanVisualEditor/PlanVisualEditor.tsx`

- Added `IconRefresh` import and `onRefresh` prop to `AdvancedToolbarProps`
- Added refresh button next to Fit View button with tooltip "Refresh canvas data"
- Wired up to `refreshData()` function from `usePlanDagCQRS` hook
- Users can now manually recover from sync issues
...
Added structured logging with timestamps for:
- Every subscription received (with operation details)
- Echo suppression decisions (with timing info)
- Sync skip decisions (with reason: `external-sync-in-progress`, `drag-in-progress`, `no-significant-changes`)
- Sync applied events (with change details)

### Projection Fixes (Previously Implemented)

Issue 2 was verified as already fixed. All three projection opening locations use `getServerInfo()` and proper event handling:
- `ProjectionsPage.tsx:147-179`
- `ProjectionNode.tsx:90-126`
- `ProjectArtefactsPage.tsx:229-263`
...
1. **TypeScript check**: Run `npm run typecheck` in frontend directory
2. **Manual testing**:
   - Open plan editor, drag nodes, verify positions sync correctly
   - Make changes in one browser tab, verify they appear in another
   - Use the refresh button to recover from any sync issues
   - Check console for structured log messages during drag operations
