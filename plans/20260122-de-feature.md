...
1. Audit the Rust workspace for lingering `mcp` and generative keywords (examples, docs, GraphQL schema). Delete or refactor files that still mention MCP endpoints and ensure `Cargo.toml` no longer pulls in MCP/rig integrations.
2. Remove or decommission the `layercake-genai` crate (or trim it to non-chat roles) so `layercake-core` no longer depends on genAI-specific modules; update imports and regeneration scripts to match.
3. Sweep the remaining `docs/` references (plans, historical notes, release docs) that mention chat/RAG/MCP, note their removal in release documentation, and rerun `cargo check`/`npm run frontend:build` once backend cleanup finishes.
