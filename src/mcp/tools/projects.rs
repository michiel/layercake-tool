...
/// List all projects
pub async fn list_projects(db: &DatabaseConnection) -> Result<Value, String> {
...
pub async fn create_project(
    ...
    let name = get_required_param(&arguments, "name")?
        .as_str()
        .ok_or("Project name must be a string")?
        .to_string();
...
pub async fn get_project(
    ...
    let project_id = get_required_param(&arguments, "project_id")?
        .as_i64()
        .ok_or("Project ID must be a number")?;

    let project = projects::Entity::find_by_id(project_id as i32)
        .one(db)
        .await
        .map_err(|e| format!("Database error: {}", e))?
...
pub async fn delete_project(
    ...
    projects::Entity::delete_by_id(project_id as i32)
        .exec(db)
        .await
