...
use layercake_core::database::entities::plan_dag_nodes;
...
impl From<layercake_core::plan_dag::PlanDagNode> for PlanDagNode {
    fn from(node: layercake_core::plan_dag::PlanDagNode) -> Self {
        Self {
            id: node.id,
            ...
            dataset_execution: node.dataset_execution.map(Into::into),
            graph_execution: node.graph_execution.map(Into::into),
        }
    }
}
